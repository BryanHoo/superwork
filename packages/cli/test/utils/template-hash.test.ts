...
describe("loadHashes / saveHashes", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "superwork-test-"));
    // Create .superwork directory for hashes file
    fs.mkdirSync(path.join(tmpDir, ".superwork"), { recursive: true });
  });
  ...
  it("loadHashes returns empty object for invalid JSON", () => {
    const hashesPath = path.join(tmpDir, ".superwork", ".template-hashes.json");
...
describe("initializeHashes", () => {
  ...
  it("returns 0 when no template directories exist", () => {
    // Create .superwork dir for saving hashes but no template files
    fs.mkdirSync(path.join(tmpDir, ".superwork"), { recursive: true });
  ...
  it("hashes files in managed directories", () => {
    // Create .superwork with a script and .claude with a command
    fs.mkdirSync(path.join(tmpDir, ".superwork", "scripts"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".superwork", "scripts", "task.py"), "print('hello')");
    ...
    const hashes = loadHashes(tmpDir);
    expect(hashes).toHaveProperty(".superwork/scripts/task.py");
    expect(hashes).toHaveProperty(".claude/commands/start.md");
  });

  it("excludes workspace and tasks directories", () => {
    fs.mkdirSync(path.join(tmpDir, ".superwork", "workspace"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".superwork", "workspace", "data.md"), "user data");
    fs.mkdirSync(path.join(tmpDir, ".superwork", "tasks"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".superwork", "tasks", "task.json"), "{}");

    const count = initializeHashes(tmpDir);
    const hashes = loadHashes(tmpDir);

    // These should be excluded
    expect(hashes).not.toHaveProperty(".superwork/workspace/data.md");
    expect(hashes).not.toHaveProperty(".superwork/tasks/task.json");
    expect(count).toBe(0);
  });

  it("excludes spec/ directory files from hashing", () => {
    fs.mkdirSync(path.join(tmpDir, ".superwork", "spec", "guides"), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, ".superwork", "spec", "frontend"), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, ".superwork", "spec", "backend"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".superwork", "spec", "guides", "index.md"), "# Guides");
    fs.writeFileSync(path.join(tmpDir, ".superwork", "spec", "frontend", "index.md"), "# Frontend");
    fs.writeFileSync(path.join(tmpDir, ".superwork", "spec", "backend", "index.md"), "# Backend");

    const count = initializeHashes(tmpDir);
    const hashes = loadHashes(tmpDir);

    // All spec/ files should be excluded
    expect(hashes).not.toHaveProperty(".superwork/spec/guides/index.md");
    expect(hashes).not.toHaveProperty(".superwork/spec/frontend/index.md");
    expect(hashes).not.toHaveProperty(".superwork/spec/backend/index.md");
    expect(count).toBe(0);
  });
});
