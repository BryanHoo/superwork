...
describe("ALL_MANAGED_DIRS", () => {
  it("starts with .superwork", () => {
    expect(ALL_MANAGED_DIRS[0]).toBe(".superwork");
  });

  it("contains .superwork plus all managed dirs", () => {
    expect(ALL_MANAGED_DIRS).toEqual([".superwork", ...new Set(PLATFORM_MANAGED_DIRS)]);
  });
...
describe("isManagedPath", () => {
  ...
  // Positive: .superwork hardcoded paths
  it("matches .superwork sub-paths", () => {
    expect(isManagedPath(".superwork/spec")).toBe(true);
    expect(isManagedPath(".superwork/tasks/some-task")).toBe(true);
  });
  ...
  it("rejects path traversal", () => {
    ...
    expect(isManagedPath("../.superwork/spec")).toBe(false);
  });
...
describe("isManagedRootDir", () => {
  ...
  it("matches .superwork", () => {
    expect(isManagedRootDir(".superwork")).toBe(true);
  });
  ...
  it("rejects sub-paths (not a root dir)", () => {
    ...
    expect(isManagedRootDir(".superwork/spec")).toBe(false);
  });
