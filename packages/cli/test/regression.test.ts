...
import {
  ...
  commonDeveloper,
  getAllScripts,
} from "../src/templates/superwork/index.js";
import {
...
describe("regression: add_session.py runtime branch context (issue-106)", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "superwork-session-"));
  });
  ...
  function writeSuperworkScripts(): void {
    const scriptsDir = path.join(tmpDir, ".superwork", "scripts");
...
`;
    fs.writeFileSync(
      path.join(tmpDir, ".superwork", "workspace", "test-dev", "index.md"),
      indexContent,
      "utf-8",
    );
  }
  ...
  function setupSessionRepo(options?: {
    ...
    writeSuperworkScripts();

    fs.mkdirSync(path.join(tmpDir, ".superwork", "workspace", "test-dev"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpDir, ".superwork", ".developer"),
      "name=test-dev\ninitialized_at=2026-03-22T00:00:00\n",
      "utf-8",
    );
    fs.writeFileSync(
      path.join(tmpDir, ".superwork", "workspace", "test-dev", "journal-1.md"),
    ...
    if (options?.taskBranch || options?.taskBaseBranch) {
      const taskDir = path.join(tmpDir, ".superwork", "tasks", "issue-106");
      fs.mkdirSync(taskDir, { recursive: true });
      fs.writeFileSync(
        path.join(tmpDir, ".superwork", ".current-task"),
        ".superwork/tasks/issue-106\n",
        "utf-8",
      );
      fs.writeFileSync(
  ...
  function runAddSession(
    ...
    const command = [
      "python3",
      JSON.stringify(path.join(tmpDir, ".superwork", "scripts", "add_session.py")),
      "--title",
...
describe("regression: task directory paths (0.2.14, 0.2.15, beta.13)", () => {
  it("[0.2.15] PATHS.TASKS is .superwork/tasks (not .superwork/workspace/*/tasks)", () => {
    expect(PATHS.TASKS).toBe(".superwork/tasks");
    expect(PATHS.TASKS).not.toContain("workspace");
  });

  it("[0.2.14] Claude agent templates do not contain hardcoded .superwork/workspace/*/tasks/ paths", () => {
    const agents = getClaudeAgents();
    for (const agent of agents) {
      expect(agent.content).not.toMatch(/\.superwork\/workspace\/[^/]+\/tasks\//);
    }
  });
...
describe("regression: resolve_task_dir path handling", () => {
  it("[beta.12] resolve_task_dir handles .superwork prefix", () => {
    // The function should recognize .superwork-prefixed paths as relative paths
    expect(commonTaskUtils).toContain('.startswith(".superwork")');
  });
...
describe("regression: shell to Python migration (beta.0)", () => {
  it("[beta.0] no .sh scripts remain in superwork templates", () => {
    const scripts = getAllScripts();
  ...
  it("[beta.3] getAllScripts covers every .py file in templates/superwork/scripts/", () => {
    ...
    const scriptsDir = path.resolve(
      ...
      "../src/templates/superwork/scripts",
    );
...
describe("regression: current-task path normalization", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "superwork-current-task-"));
  });
  ...
  function setupTaskRepo(taskRef = ".superwork\\tasks\\issue-106"): void {
    writeSuperworkScripts();
    writeProjectFile(
      path.join(".superwork", ".developer"),
      "name=test-dev\ninitialized_at=2026-03-27T00:00:00\n",
    );
    writeProjectFile(path.join(".superwork", "workflow.md"), "# Workflow\n");
    writeProjectFile(
      path.join(".superwork", "spec", "guides", "index.md"),
      "# Guides\n",
    );
    writeProjectFile(path.join(".superwork", ".current-task"), `${taskRef}\n`);
    writeProjectFile(
      path.join(".superwork", "tasks", "issue-106", "task.json"),
      JSON.stringify(
        {
          title: "Issue 106 task",
          status: "in_progress",
    ...
    writeProjectFile(
      path.join(".superwork", "tasks", "issue-106", "prd.md"),
      "# PRD\n",
    );
    writeProjectFile(
      path.join(".superwork", "tasks", "issue-106", "implement.jsonl"),
  ...
  it("[current-task] task.py start canonicalizes Windows-style task refs before writing", () => {
    setupTaskRepo("");
    const taskScriptPath = path.join(tmpDir, ".superwork", "scripts", "task.py");

    const output = execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} start ${JSON.stringify(".superwork\\\\tasks\\\\issue-106")}`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
      },
    ...
    expect(output).toContain(".superwork/tasks/issue-106");
    expect(
      fs.readFileSync(path.join(tmpDir, ".superwork", ".current-task"), "utf-8").trim(),
    ).toBe(".superwork/tasks/issue-106");
  });
...
describe("regression: migration manifest consistency", () => {
  ...
  it("[#57] shell archive migrations use rename type with correct from/to paths", () => {
    ...
    for (const m of shellArchives) {
      ...
      expect(m.from).toMatch(/\.superwork\/scripts\/.*\.sh$/);
      expect(m.to).toMatch(/\.superwork\/scripts-shell-archive\/.*\.sh$/);
  ...
  it("[0.2.14] command namespace migration renames exist", () => {
    ...
    // Should include commands moved to superwork/ subdirectory
...
describe("regression: parse_simple_yaml Python execution (0.3.8)", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "superwork-yaml-py-"));
