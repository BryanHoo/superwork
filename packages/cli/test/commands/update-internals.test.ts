...
describe("cleanupEmptyDirs", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "superwork-cleanup-"));
  });
  ...
  it("[CR#1] does not delete .superwork root even if empty", () => {
    fs.mkdirSync(path.join(tmpDir, ".superwork"), { recursive: true });
    cleanupEmptyDirs(tmpDir, ".superwork");
    expect(fs.existsSync(path.join(tmpDir, ".superwork"))).toBe(true);
  });

  it("[CR#1] does not delete .codex root even if empty", () => {
  ...
  it("recursively cleans parent directories but stops at root", () => {
    // Create .superwork/scripts/multi_agent/ (all empty)
    fs.mkdirSync(path.join(tmpDir, ".superwork", "scripts", "multi_agent"), {
      recursive: true,
    });
    cleanupEmptyDirs(tmpDir, ".superwork/scripts/multi_agent");

    // multi_agent and scripts should be removed (both empty)
    expect(
      fs.existsSync(
        path.join(tmpDir, ".superwork", "scripts", "multi_agent"),
      ),
    ).toBe(false);
    expect(
      fs.existsSync(path.join(tmpDir, ".superwork", "scripts")),
    ).toBe(false);
    // .superwork root must survive
    expect(fs.existsSync(path.join(tmpDir, ".superwork"))).toBe(true);
  });
...
describe("loadUpdateSkipPaths", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "superwork-skip-"));
    fs.mkdirSync(path.join(tmpDir, ".superwork"), { recursive: true });
  });
  ...
  it("strips double quotes from skip paths", () => {
    fs.writeFileSync(
      path.join(tmpDir, ".superwork", "config.yaml"),
      'update:\n  skip:\n    - ".claude/commands/"\n',
    );
...
describe("sortMigrationsForExecution", () => {
  ...
  it("puts rename-dir before rename and delete", () => {
    const items = [
      ...
      { type: "rename-dir" as const, from: ".superwork/old", to: ".superwork/new" },
  ...
  it("sorts rename-dir by path depth (deeper first)", () => {
    const items = [
      { type: "rename-dir" as const, from: ".superwork/a", to: ".superwork/x" },
      {
        type: "rename-dir" as const,
        from: ".superwork/a/b/c",
        to: ".superwork/x/y/z",
      },
      { type: "rename-dir" as const, from: ".superwork/a/b", to: ".superwork/x/y" },
    ];
    const sorted = sortMigrationsForExecution(items);
    expect(sorted[0].from).toBe(".superwork/a/b/c"); // depth 4
    expect(sorted[1].from).toBe(".superwork/a/b"); // depth 3
    expect(sorted[2].from).toBe(".superwork/a"); // depth 2
  });
