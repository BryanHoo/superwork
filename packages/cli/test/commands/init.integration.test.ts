...
describe("init() integration", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "superwork-init-int-"));
  ...
  it("#14 monorepo: writes packages section to config.yaml", async () => {
    setupPnpmWorkspace(tmpDir, [
      { rel: "packages/cli", name: "@superwork/cli" },
      { rel: "packages/docs", name: "@superwork/docs" },
    ]);
  ...
  it("#15 monorepo: bootstrap task references per-package spec paths", async () => {
    ...
    expect(taskJson.relatedFiles).toContain(".superwork/spec/core/");
    expect(taskJson.relatedFiles).toContain(".superwork/spec/ui/");
  ...
  it("#17 --monorepo without workspace config exits with error", async () => {
    ...
    // Should NOT create .superwork/ (early return)
