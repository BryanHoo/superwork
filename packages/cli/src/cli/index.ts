...
/**
 * Check if a Superwork update is available (compare project version with CLI version)
 */
function checkForUpdates(cwd: string): void {
  ...
  if (comparison > 0) {
    ...
    console.log(
      chalk.yellow(
        `\n⚠️  Superwork update available: ${projectVersion} → ${cliVersion}`,
      ),
    );
    console.log(chalk.gray(`   Run: superwork update\n`));
...
// Check for updates at CLI startup (only if .superwork exists)
...
program
  .command("update")
  .description("Update superwork configuration and commands to latest version")
