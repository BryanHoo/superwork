...
```bash
python3 ./.superwork/scripts/get_context.py --mode packages  # Discover available spec layers
cat .superwork/spec/guides/index.md                          # Thinking guides
```
...
```bash
python3 ./.superwork/scripts/multi_agent/plan.py \
  --name "<feature-name>" \
...
```bash
python3 ./.superwork/scripts/multi_agent/start.py "$TASK_DIR" --platform codex
```
...
```bash
TASK_DIR=$(python3 ./.superwork/scripts/task.py create "<title>" --slug <task-name>)
```

#### Step 2: Configure Task

```bash
python3 ./.superwork/scripts/task.py init-context "$TASK_DIR" <dev_type>
python3 ./.superwork/scripts/task.py set-branch "$TASK_DIR" feature/<name>
python3 ./.superwork/scripts/task.py set-scope "$TASK_DIR" <scope>
```
...
```bash
python3 ./.superwork/scripts/task.py validate "$TASK_DIR"
python3 ./.superwork/scripts/multi_agent/start.py "$TASK_DIR" --platform codex
```
...
| Skill | Description |
|-------|-------------|
| `$superwork-parallel` | Start Multi-Agent Pipeline (this skill) |
| `$superwork-start` | Start normal development mode (single process) |
| `$superwork-record-session` | Record session progress |
| `$superwork-finish-work` | Pre-completion checklist |
...
```bash
python3 ./.superwork/scripts/multi_agent/status.py                    # Overview
python3 ./.superwork/scripts/multi_agent/status.py --log <name>       # View log
python3 ./.superwork/scripts/multi_agent/status.py --watch <name>     # Real-time monitoring
python3 ./.superwork/scripts/multi_agent/cleanup.py <branch>          # Cleanup worktree
```
...
- **Don't execute `git commit` manually in this flow** - let `create-pr` commit after `finish` verification passes
