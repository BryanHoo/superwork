...
- Use `$spec-plan` to decide what behaviors need tests.
- Use `$execute-plan` to sequence TDD across a larger task.
- Use `$debug-root-cause` first when the task begins with a failing bug.
