...
- Read repository guidance first with `$before-dev`.
...
- If the change becomes architectural or ambiguous, pause and return to `$spec-plan` or `$brainstorm`.
...
3. Use `$check` for a guideline pass.
4. Use `$check-cross-layer` when the change spans multiple layers or contracts.
5. Use `$finish-work` before handing off for commit.

If the task was a bug fix with reusable lessons, follow with `$break-loop`.
