...
```bash
cat .superwork/spec/<category>/<file>.md
```
...
```
Development Flow:
  Learn something → $superwork-update-spec → Knowledge captured
       ↑                                  ↓
  $superwork-break-loop ←──────────────────── Future sessions benefit
  (deep bug analysis)
```

- `$superwork-break-loop` - Analyzes bugs deeply, often reveals spec updates needed
- `$superwork-update-spec` - Actually makes the updates (this skill)
- `$superwork-finish-work` - Reminds you to check if specs need updates
