...
Triggered from `$superwork-start` when the user describes a development task, especially when:
...
```bash
TASK_DIR=$(python3 ./.superwork/scripts/task.py create "brainstorm: <short goal>" --slug <auto>)
```
...
```bash
# Create child tasks
CHILD1=$(python3 ./.superwork/scripts/task.py create "Child task 1" --slug child1 --parent "$TASK_DIR")
CHILD2=$(python3 ./.superwork/scripts/task.py create "Child task 2" --slug child2 --parent "$TASK_DIR")

# Or link existing tasks
python3 ./.superwork/scripts/task.py add-subtask "$TASK_DIR" "$CHILD_DIR"
```
...
After brainstorm completes (Step 8 confirmation approved), the flow continues through `$superwork-spec-plan` before implementation:
...
| Command | When to Use |
|---------|-------------|
| `$superwork-start` | Entry point that triggers `superwork-brainstorm` |
| `$superwork-spec-plan` | After requirements are approved, before coding |
| `$superwork-execute-plan` | After the plan is ready |
| `$superwork-finish-work` | After implementation is complete |
| `$superwork-update-spec` | If new patterns emerge during work |
