...
If you skip, say so explicitly and go straight to `$before-dev` and `$tdd-core`.
...
4. If requirements are still unstable, return to `$brainstorm` instead of forcing a plan.
...
- Use `$execute-plan` to implement the plan.
- Use `$tdd-core` inside each behavior-changing step.
- If this is a bug, start with `$debug-root-cause` before planning or coding.
