---
name: superwork-onboard
description: "Interactive three-part onboarding for new team members to the Superwork AI-assisted workflow system. Covers core philosophy (AI memory, project-specific knowledge, context drift), system structure and command deep-dives, real-world workflow examples, and guideline customization. Use when a new developer joins the project, someone needs to understand the Superwork workflow, or project guidelines need initial setup."
---
...
**The Solution**: The `.superwork/workspace/` system captures what happened in each session - what was done, what was learned, what problems were solved. The `$superwork-start` skill reads this history at session start, giving AI "artificial memory."
...
**The Solution**: The `.superwork/spec/` directory contains project-specific guidelines. The `$superwork-before-dev` skill injects this specialized knowledge into AI context before coding starts.
...
**The Solution**: The `$superwork-check` and `$superwork-check-cross-layer` skills re-verify code against guidelines AFTER writing, catching drift that occurred during development. The `$superwork-finish-work` skill does a final holistic review.
...
### $superwork-start - Restore AI Memory
...
**WHY THIS MATTERS**:
- Without $superwork-start: AI is blind. It might work on wrong branch, conflict with others' work, or redo already-completed work.
- With $superwork-start: AI knows project context, can continue where previous session left off, avoids conflicts.

---

### $superwork-before-dev - Inject Specialized Knowledge
...
### $superwork-check - Combat Context Drift
...
**WHY THIS MATTERS**:
- Without Superwork checks: Context drift goes unnoticed, code quality degrades.
- With Superwork checks: Drift is caught and corrected before commit.
...
### $superwork-finish-work - Holistic Pre-Commit Review

**WHY IT EXISTS**:
The `$superwork-check` skill focuses on code quality within a single layer. Cross-layer changes should additionally use `$superwork-check-cross-layer`.
...
### $superwork-record-session - Persist Memory for Future

**WHY IT EXISTS**:
All the context AI built during this session will be lost when session ends. The next session's `$superwork-start` needs this information.
...
**[1/8] $superwork-start** - AI needs project context before touching code
...
**[7/8] Run final verification and commit** - Commit once checks and testing pass
**[8/8] $superwork-record-session** - Persist memory for future sessions

### Example 2: Planning Session (No Code)

**[1/4] $superwork-start** - Context needed even for non-coding work
**[2/4] python3 ./.superwork/scripts/task.py create "Planning task" --slug planning-task** - Planning is valuable work
**[3/4] Review docs, create subtask list** - Actual planning work
**[4/4] $superwork-record-session (with --summary)** - Planning decisions must be recorded
...
**[1/5] $superwork-start** - Clear baseline before major changes
**[2/5] Plan phases** - Break into verifiable chunks
**[3/5] Execute phase by phase with `$superwork-check` / `$superwork-check-cross-layer` after each** - Incremental verification
**[4/5] $superwork-finish-work** - Check if new patterns should be documented
...
2. **Guidelines before code** - `$superwork-before-dev` injects project knowledge.
3. **Check after code** - `$superwork-check` and `$superwork-check-cross-layer` catch context drift.
...
Check if `.superwork/spec/` contains empty templates or customized guidelines:

```bash
# Check if files are still empty templates (look for placeholder text)
grep -l "To be filled by the team" .superwork/spec/backend/*.md 2>/dev/null | wc -l
grep -l "To be filled by the team" .superwork/spec/frontend/*.md 2>/dev/null | wc -l
```
...
If guidelines are empty templates (contain "To be filled by the team"), this is the first time using Superwork in this project.

Explain to the developer:

"I see that the development guidelines in `.superwork/spec/` are still empty templates. This is normal for a new Superwork setup!

The templates contain placeholder text that needs to be replaced with YOUR project's actual conventions. Without this, `$superwork-before-dev` won't provide useful guidance.
...
For example, for `.superwork/spec/backend/database-guidelines.md`:
...
"Great! Your team has already customized the development guidelines. You can start using `$superwork-before-dev` right away.

I recommend reading through `.superwork/spec/` to familiarize yourself with the team's coding standards."
...
```bash
python3 ./.superwork/scripts/task.py create "Fill spec guidelines" --slug fill-spec-guidelines
```
...
"You're now onboarded to the Superwork workflow system! Here's what we covered:
...
1. Run `$superwork-record-session` to record this onboard session
2. [If guidelines empty] Start filling in `.superwork/spec/` guidelines
