---
name: superwork-integrate-skill
description: "Adapts an external skill into project-specific development guidelines in .superwork/spec/. Creates guideline sections, code example templates with .template suffix, and updates spec indexes. Use when integrating an external skill, adding a new skill's patterns to project conventions, or incorporating third-party skill best practices into .superwork/spec/ documentation."
---
...
```
$superwork-integrate-skill <skill-name>
```

**Examples**:
```
$superwork-integrate-skill frontend-design
$superwork-integrate-skill mcp-builder
```
...
>
> - Guidelines content -> Write to `.superwork/spec/{target}/doc.md`
> - Code examples -> Place in `.superwork/spec/{target}/examples/skills/<skill-name>/`
...
- `.agents/skills/superwork-<skill-name>/SKILL.md` in the repository when integrating a project-local Superwork skill
...
|----------------|-------------------|
| UI/Frontend (`frontend-design`, `web-artifacts-builder`) | `.superwork/spec/frontend/` |
| Backend/API (`mcp-builder`) | `.superwork/spec/backend/` |
| Documentation (`doc-coauthoring`, `docx`, `pdf`) | `.superwork/` or create dedicated guidelines |
| Testing (`webapp-testing`) | `.superwork/spec/frontend/` (E2E) |
...
- **Integration target**: `.superwork/spec/{target}/`
...
| Type | Path |
|------|------|
| Guidelines doc | `.superwork/spec/{target}/doc.md` (section: `skill-<name>`) |
| Code examples | `.superwork/spec/{target}/examples/skills/<name>/` |
| Index update | `.superwork/spec/{target}/index.md` |
...
```bash
$superwork-create-command use-<skill-name> Use <skill-name> skill following project guidelines
```
...
| `doc-coauthoring` | `.superwork/` | N/A (documentation workflow only) |
