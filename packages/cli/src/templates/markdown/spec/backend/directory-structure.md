...
├── worktree.yaml        # Worktree config (Superwork-specific)
...
Files that are copied directly from Superwork project to user projects:
...
| `.superwork/workspace/index.md` | `.superwork/workspace/index.md` | Direct copy |
...
|----------------|-------------|--------|
| `src/templates/markdown/spec/**/*.md.txt` | `.superwork/spec/**/*.md` | User fills with project-specific content |
| `src/templates/markdown/worktree.yaml.txt` | `.superwork/worktree.yaml` | Language-agnostic template |
...
```typescript
// Get path to .superwork/ (works in dev and production)
getSuperworkSourcePath(): string

// Read file from .superwork/
readSuperworkFile(relativePath: string): string

// Copy directory from .superwork/ with executable scripts
copySuperworkDir(srcRelativePath: string, destPath: string, options?: { executable?: boolean }): void
```
...
const tempDir = path.join(os.tmpdir(), `superwork-template-${Date.now()}`);
