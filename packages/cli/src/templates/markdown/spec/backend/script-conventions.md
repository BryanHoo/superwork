...
> Standards for Python scripts in the `.superwork/scripts/` directory.
...
# Constants
DIR_WORKFLOW = ".superwork"
DIR_SCRIPTS = "scripts"
DIR_TASKS = "tasks"

def get_repo_root() -> Path | None:
    """Find repository root by looking for .superwork directory."""
...
print("Run: python3 ./.superwork/scripts/init_developer.py <name>")
...
path = Path(".superwork") / "scripts" / "task.py"

# Bad - Unix-only
path = ".superwork/scripts/task.py"
```
...
hooks:
  after_create:
    - "python3 .superwork/scripts/hooks/my_hook.py create"
  after_start:
    - "python3 .superwork/scripts/hooks/my_hook.py start"
  after_archive:
    - "python3 .superwork/scripts/hooks/my_hook.py archive"
```
...
1. Store config in a **gitignored** local file (e.g., `.superwork/hooks.local.json`)
2. Read config at startup, fail with clear message if missing
3. Keep the script itself committable (no hardcoded secrets)

```python
# .superwork/scripts/hooks/my_hook.py — committable, no secrets
CONFIG = _load_config()  # reads from .superwork/hooks.local.json (gitignored)
...
Scripts that modify `.superwork/` tracked files should auto-commit their changes to keep the workspace clean. Use a `--no-commit` flag for opt-out.

### Convention: Auto-Commit After Mutation

```python
def _auto_commit(scope: str, message: str, repo_root: Path) -> None:
    """Stage and commit changes in a specific .superwork/ subdirectory."""
...
- `add_session.py` — commits `.superwork/workspace` + `.superwork/tasks` after recording a session
- `task.py archive` — commits `.superwork/tasks` after archiving a task
...
```yaml
# Auto-detected monorepo packages (written by superwork init)
packages:
  cli:
    path: packages/cli
  docs-site:
...
update:
  skip:
    - .claude/commands/superwork/my-custom.md
...
# Usage
if not repo_root:
    error("Not in a Superwork project (no .superwork directory found)")
    sys.exit(1)
```
...
See `.superwork/scripts/task.py` for a comprehensive example with:
...
> **Historical Context**: Scripts were migrated from Bash to Python in v0.3.0 for cross-platform compatibility. The old shell scripts are archived in `.superwork/scripts-shell-archive/` (if preserved).
