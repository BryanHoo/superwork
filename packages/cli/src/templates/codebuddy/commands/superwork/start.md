...
### Step 1: Understand Superwork Workflow `[AI]`

First, read the workflow guide to understand the development process:

```bash
cat .superwork/workflow.md  # Development process, conventions, and quick start guide
```
...
```bash
cat .superwork/spec/<package>/<layer>/index.md   # Package-specific guidelines
cat .superwork/spec/guides/index.md              # Thinking guides (always read)
```
...
> See `/superwork:brainstorm` Step 8 for details.
...
2. If code was changed, remind user to run `/superwork:finish-work`
...
For complex or vague tasks, **automatically start the brainstorm process** — do NOT skip directly to implementation. Use `/superwork:brainstorm`.
...
```bash
TASK_DIR=$(python3 ./.superwork/scripts/task.py create "<title>" --slug <name>)
```
...
1. Relevant spec files in `.superwork/spec/`
...
```bash
python3 ./.superwork/scripts/task.py init-context "$TASK_DIR" <type>
...
python3 ./.superwork/scripts/task.py add-context "$TASK_DIR" implement "<path>" "<reason>"
python3 ./.superwork/scripts/task.py add-context "$TASK_DIR" check "<path>" "<reason>"
```

**Step 7: Activate Task** `[AI]`

```bash
python3 ./.superwork/scripts/task.py start "$TASK_DIR"
```
...
3. Remind user to:
   - Test the changes
   - Commit when ready
   - Run `/superwork:record-session` to record this session
...
| Command | Description |
|---------|-------------|
| `/superwork:start` | Start development session (this command) |
| `/superwork:brainstorm` | Clarify vague requirements before implementation |
| `/superwork:before-dev` | Read development guidelines |
| `/superwork:check` | Check code quality |
| `/superwork:check-cross-layer` | Cross-layer verification |
| `/superwork:finish-work` | Pre-commit checklist |
| `/superwork:record-session` | Record session progress |
...
| `python3 ./.superwork/scripts/task.py archive <name>` | Archive task |
| `python3 ./.superwork/scripts/get_context.py` | Get session context |

---

## Platform Detection

Superwork auto-detects your platform based on config directories. For CodeBuddy users, ensure detection works correctly:
...
```bash
export SUPERWORK_PLATFORM=codebuddy
```

Or prefix commands:

```bash
SUPERWORK_PLATFORM=codebuddy python3 ./.superwork/scripts/task.py list
```
...
> Before ending this session, please run `/superwork:record-session` to record what we accomplished.
