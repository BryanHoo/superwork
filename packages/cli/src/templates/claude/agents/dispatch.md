...
Current Task is specified by `.superwork/.current-task` file, content is the relative path to task directory.

Task directory path format: `.superwork/tasks/{MM}-{DD}-{name}/`
...
Read `.superwork/.current-task` to get current task directory path:

```bash
TASK_DIR=$(cat .superwork/.current-task)
# e.g.: .superwork/tasks/02-03-my-feature
```
...
```bash
python3 ./.superwork/scripts/multi_agent/create_pr.py
```
