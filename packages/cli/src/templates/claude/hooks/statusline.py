...
# -*- coding: utf-8 -*-
"""
Superwork StatusLine — project-level status display for Claude Code.

Reads Claude Code session JSON from stdin + Superwork task data from filesystem.
Outputs 1-2 lines:
...
def _find_superwork_dir() -> Path | None:
    """Walk up from cwd to find .superwork/ directory."""
    current = Path.cwd()
    for parent in [current, *current.parents]:
        candidate = parent / ".superwork"
        if candidate.is_dir():
            return candidate
    return None


def _get_current_task(superwork_dir: Path) -> dict | None:
    """Load current task info. Returns dict with title/status/priority or None."""
    task_ref = _normalize_task_ref(_read_text(superwork_dir / ".current-task"))
    if not task_ref:
        return None

    # Resolve task directory
    task_path = _resolve_task_dir(superwork_dir, task_ref)
...
def _count_active_tasks(superwork_dir: Path) -> int:
    """Count non-archived task directories with valid task.json."""
    tasks_dir = superwork_dir / "tasks"
...
def _get_developer(superwork_dir: Path) -> str:
    content = _read_text(superwork_dir / ".developer")
    if not content:
        return "unknown"
...
def main() -> None:
    ...
    superwork_dir = _find_superwork_dir()
    SEP = " \033[90m·\033[0m "

    # --- Superwork data ---
    task = _get_current_task(superwork_dir) if superwork_dir else None
    dev = _get_developer(superwork_dir) if superwork_dir else ""
    task_count = _count_active_tasks(superwork_dir) if superwork_dir else 0
