...
def _load_superwork_config(superwork_dir: Path) -> tuple:
    ...
    try:
        ...
        repo_root = superwork_dir.parent
...
def _check_legacy_spec(superwork_dir: Path, is_mono: bool, packages: dict) -> str | None:
...
def main():
    ...
    is_mono, packages, scope_config, task_pkg, default_pkg = _load_superwork_config(superwork_dir)
...
""")
    ...
    legacy_warning = _check_legacy_spec(superwork_dir, is_mono, packages)
    if legacy_warning:
    ...
    start_md = read_file(
        claude_dir / "commands" / "superwork" / "start.md", "No start.md found"
    )
