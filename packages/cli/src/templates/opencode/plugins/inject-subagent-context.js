/**
 * Superwork Context Injection Plugin
 *
...
import { SuperworkContext, debugLog } from "../lib/superwork-context.js"
...
function getCheckContext(ctx, taskDir) {
  ...
  if (entries.length > 0) {
    ...
    const checkFiles = [
      [".opencode/commands/superwork/finish-work.md", "Finish work checklist"],
      [".opencode/commands/superwork/check-cross-layer.md", "Cross-layer check spec"],
      [".opencode/commands/superwork/check.md", "Check spec"],
    ]
...
function getFinishContext(ctx, taskDir) {
  ...
  if (entries.length > 0) {
    ...
    const finishWork = ctx.readProjectFile(".opencode/commands/superwork/finish-work.md")
    if (finishWork) {
      parts.push(`=== .opencode/commands/superwork/finish-work.md (Finish checklist) ===\n${finishWork}`)
    }
  }

  // 2. Spec update process (for active spec sync)
  const updateSpec = ctx.readProjectFile(".opencode/commands/superwork/update-spec.md")
  if (updateSpec) {
    parts.push(`=== .opencode/commands/superwork/update-spec.md (Spec update process) ===\n${updateSpec}`)
  }
...
- Spec files: \`.superwork/spec/**/*.md\`
- Known issues: \`.superwork/big-question/\`
...
export default async ({ directory }) => {
  ...
  return {
    ...
    //
    // SOLUTION: Superwork + OpenCode users must install oh-my-opencode (omo)
