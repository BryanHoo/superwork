/* global process */
/**
 * Superwork Session Start Plugin
 *
...
import { SuperworkContext, contextCollector, debugLog } from "../lib/superwork-context.js"
...
function getTaskStatus(ctx) {
  ...
  if (taskStatus === "completed") {
    ...
    return `Status: COMPLETED\nTask: ${taskTitle}\nNext: Archive with \`python3 ./.superwork/scripts/task.py archive ${dirName}\` or start a new task`
  }
...
/**
 * Load Superwork config for session-start decisions.
 * Calls get_context.py --mode packages --json for reliable config data.
 */
function loadSuperworkConfig(directory) {
  const scriptPath = join(directory, ".superwork", "scripts", "get_context.py")
  ...
  try {
    ...
    debugLog("session", "loadSuperworkConfig error:", e.message)
    return { isMonorepo: false, packages: {}, specScope: null, activeTaskPackage: null, defaultPackage: null }
  }
}
...
function checkLegacySpec(directory, config) {
  ...
  const specDir = join(directory, ".superwork", "spec")
...
Read and follow all instructions below carefully.
  ...
  const contextScript = join(superworkDir, "scripts", "get_context.py")
  ...
  // 3. Workflow Guide
  const workflow = ctx.readProjectFile(".superwork/workflow.md")
  if (workflow) {
    parts.push("<workflow>")
    parts.push(workflow)
    parts.push("</workflow>")
  ...
  let startMd = ctx.readFile(join(claudeDir, "commands", "superwork", "start.md"))
  if (!startMd) {
    startMd = ctx.readFile(join(opencodeDir, "commands", "superwork", "start.md"))
  }
  if (startMd) {
...
export default async ({ directory }) => {
  const ctx = new SuperworkContext(directory)
