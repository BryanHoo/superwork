...
// Debug logging
const DEBUG_LOG = "/tmp/superwork-plugin-debug.log"
...
export class SuperworkContext {
  constructor(directory) {
    ...
    debugLog("context", "SuperworkContext initialized", { directory })
  }
  ...
  // ============================================================
  // Superwork Project Detection
  // ============================================================

  /**
   * Check if this is a Superwork-managed project
   */
  isSuperworkProject() {
    return existsSync(join(this.directory, ".superwork"))
  }

  /**
   * Get current task directory from .superwork/.current-task
   */
  getCurrentTask() {
    try {
      const currentTaskPath = join(this.directory, ".superwork", ".current-task")
  ...
  resolveTaskDir(taskRef) {
    ...
    return join(this.directory, ".superwork", "tasks", normalized)
  }
  ...
  shouldSkipHook(hookName) {
    // Not a Superwork project? Skip.
    if (!this.isSuperworkProject()) {
      debugLog("context", `shouldSkipHook(${hookName}): skip - not Superwork project`)
      return true
    }
