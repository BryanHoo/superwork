...
after initializing Superwork for the first time.
...
Prerequisites:
    - .superwork/.developer must exist (run init_developer.py first)

Creates:
    .superwork/tasks/00-bootstrap-guidelines/
...
|------|------------------|
| `.superwork/{spec_base}/backend/directory-structure.md` | Where different file types go (routes, services, utils) |
| `.superwork/{spec_base}/backend/database-guidelines.md` | ORM, migrations, query patterns, naming conventions |
| `.superwork/{spec_base}/backend/error-handling.md` | How errors are caught, logged, and returned |
| `.superwork/{spec_base}/backend/logging-guidelines.md` | Log levels, format, what to log |
| `.superwork/{spec_base}/backend/quality-guidelines.md` | Code review standards, testing requirements |
"""
...
| File | What to Document |
|------|------------------|
| `.superwork/{spec_base}/frontend/directory-structure.md` | Component/page/hook organization |
| `.superwork/{spec_base}/frontend/component-guidelines.md` | Component patterns, props conventions |
| `.superwork/{spec_base}/frontend/hook-guidelines.md` | Custom hook naming, patterns |
| `.superwork/{spec_base}/frontend/state-management.md` | State library, patterns, what goes where |
| `.superwork/{spec_base}/frontend/type-safety.md` | TypeScript conventions, type organization |
| `.superwork/{spec_base}/frontend/quality-guidelines.md` | Linting, testing, accessibility |
"""
...
def write_task_json(task_dir: Path, developer: str, project_type: str, spec_base: str) -> None:
    ...
    if project_type == "frontend":
        ...
        related_files = [f".superwork/{spec_base}/frontend/"]
    elif project_type == "backend":
        subtasks = [
            {"name": "Fill backend guidelines", "status": "pending"},
            {"name": "Add code examples", "status": "pending"},
        ]
        related_files = [f".superwork/{spec_base}/backend/"]
    else:  # fullstack
        subtasks = [
            {"name": "Fill backend guidelines", "status": "pending"},
            {"name": "Fill frontend guidelines", "status": "pending"},
        ...
        related_files = [f".superwork/{spec_base}/backend/", f".superwork/{spec_base}/frontend/"]
    ...
    task_data = {
        ...
        "notes": f"First-time setup task created by superwork init ({project_type} project)",
        "meta": {},
    }
