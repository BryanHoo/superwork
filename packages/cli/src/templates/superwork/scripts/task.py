...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not full_path.is_dir():
        ...
        print("Hint: Use task name (e.g., 'my-task') or full path (e.g., '.superwork/tasks/01-31-my-task')")
        return 1
...
Examples:
  ...
  python3 task.py create "Child task" --slug child --parent .superwork/tasks/01-21-parent
  python3 task.py init-context .superwork/tasks/01-21-add-login backend
  python3 task.py init-context .superwork/tasks/01-21-add-login backend --package cli
  python3 task.py add-context <dir> implement .superwork/spec/cli/backend/auth.md "Auth guidelines"
  python3 task.py set-branch <dir> task/add-login
  python3 task.py start .superwork/tasks/01-21-add-login
