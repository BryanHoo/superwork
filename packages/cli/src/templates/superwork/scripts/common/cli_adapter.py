...
class CLIAdapter:
    ...
    _AGENT_NAME_MAP: ClassVar[dict[Platform, dict[str, str]]] = {
        ...
        "opencode": {
            "plan": "superwork-plan",  # 'plan' is built-in in OpenCode
        },
    }
    ...
    def get_agent_name(self, agent: str) -> str:
        ...
        Returns:
            Platform-specific agent name (e.g., 'superwork-plan' for OpenCode)
        """
    ...
    def get_commands_path(self, project_root: Path, *parts: str) -> Path:
        ...
        Args:
            project_root: Project root directory
            *parts: Additional path parts (e.g., 'superwork', 'finish-work.md')

        Returns:
            Path to commands directory or file

        Note:
            Cursor uses prefix naming: .cursor/commands/superwork-<name>.md
            Antigravity uses workflow directory: .agent/workflows/<name>.md
            Claude/OpenCode use subdirectory: .claude/commands/superwork/<name>.md
        """
        if self.platform in ("antigravity", "kilo"):
            workflow_dir = self.get_config_dir(project_root) / "workflows"
            if not parts:
                return workflow_dir
            if len(parts) >= 2 and parts[0] == "superwork":
                filename = parts[-1]
        ...
        if self.platform == "cursor" and len(parts) >= 2 and parts[0] == "superwork":
            # Convert superwork/<name>.md to superwork-<name>.md
            filename = parts[-1]
            return (
                self.get_config_dir(project_root) / "commands" / f"superwork-{filename}"
            )

        return self.get_config_dir(project_root) / "commands" / Path(*parts)

    def get_superwork_command_path(self, name: str) -> str:
        """Get relative path to a superwork command file.
        ...
        Note:
            ...
            Gemini: .gemini/commands/superwork/<name>.toml
            Antigravity: .agent/workflows/<name>.md
            Others: .{platform}/commands/superwork/<name>.md
        """
        if self.platform == "cursor":
            return f".cursor/commands/superwork-{name}.md"
        elif self.platform == "codex":
            return f".agents/skills/{name}/SKILL.md"
        elif self.platform == "kiro":
            return f".kiro/skills/{name}/SKILL.md"
        elif self.platform == "gemini":
            return f".gemini/commands/superwork/{name}.toml"
        elif self.platform == "antigravity":
...
def detect_platform(project_root: Path) -> Platform:
    ...
    Detection order:
    1. SUPERWORK_PLATFORM environment variable (if set)
