...
def _auto_commit_workspace(repo_root: Path) -> None:
    """Stage .superwork/workspace and .superwork/tasks, then commit with a configured message."""
    commit_msg = get_session_commit_message(repo_root)
    subprocess.run(
        ["git", "add", "-A", ".superwork/workspace", ".superwork/tasks"],
        cwd=repo_root,
        capture_output=True,
    )
    # Check if there are staged changes
    result = subprocess.run(
        ["git", "diff", "--cached", "--quiet", "--", ".superwork/workspace", ".superwork/tasks"],
        cwd=repo_root,
    )
