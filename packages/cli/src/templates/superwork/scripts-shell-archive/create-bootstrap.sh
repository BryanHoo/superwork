...
#
# Usage:
#   ./.superwork/scripts/create-bootstrap.sh [project-type]
#
# Arguments:
#   project-type: frontend | backend | fullstack (default: fullstack)
#
# Prerequisites:
#   - .superwork/.developer must exist (run init-developer.sh first)
#
# Creates:
...
```bash
./.superwork/scripts/task.sh finish
./.superwork/scripts/task.sh archive 00-bootstrap-guidelines
```
  ...
  cat > "$dir/task.json" << EOF
{
  ...
  "notes": "First-time setup task created by superwork init ($project_type project)"
}
EOF
}
