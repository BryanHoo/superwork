...
import {
  ...
  gitignoreTemplate,
} from "../templates/superwork/index.js";
...
/**
 * Load update.skip paths from .superwork/config.yaml
 *
 ...
 *
 * Old Superwork versions used `.agents/skills/` as codex's configDir.
 * New versions use `.codex/` for Codex-specific config and `.agents/skills/`
 * as a shared layer.
 *
 * Detection: Superwork-tracked hashes contain `.agents/skills/` entries
...
function classifyMigrations(
  ...
  for (const item of migrations) {
    ...
    // rename/rename-dir are allowed to target protected paths (e.g., 0.2.0 renames into .superwork/workspace)
    if (
      item.to &&
 ...
 * Recursively removes empty parent directories up to .superwork root
 */
/** @internal Exported for testing only */
export function cleanupEmptyDirs(cwd: string, dirPath: string): void {
  ...
  // Safety: never delete managed root directories themselves (e.g., .claude, .superwork)
...
export async function update(options: UpdateOptions): Promise<void> {
  ...
  // Check if Superwork is initialized
  if (!fs.existsSync(path.join(cwd, DIR_NAMES.WORKFLOW))) {
    console.log(chalk.red("Error: Superwork not initialized in this directory."));
    console.log(chalk.gray("Run 'superwork init' first."));
    return;
  }
  ...
  if (cliVsProject < 0) {
    ...
    if (!options.allowDowngrade) {
      ...
      console.log(
        chalk.gray(`  2. Force downgrade: superwork update --allow-downgrade\n`),
      );
      return;
    }
  ...
  if (isUnknownVersion) {
    console.log(
      chalk.yellow(
        "⚠️  No version file found. Skipping migrations — run superwork init to fix.",
      ),
    );
  ...
  // Detect legacy Codex (has .agents/skills/ tracked by Superwork but no .codex/)
  ...
  if (options.migrate && classifiedMigrations) {
    ...
    // traces-*.md files are in .superwork/workspace/{developer}/ with variable developer names
  ...
  if (cliVsProject > 0 && projectVersion !== "unknown") {
    ...
    if (metadata.breaking && metadata.migrationGuides.length > 0) {
      ...
      if (!fs.existsSync(taskDir)) {
        ...
        console.log(
          chalk.gray(
            "Use AI to help: Ask Claude/Codex to read the task and fix your custom files.",
          ),
        );
      }
    }
