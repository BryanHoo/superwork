...
|------|------------------|
| \`.superwork/spec/backend/directory-structure.md\` | Where different file types go (routes, services, utils) |
| \`.superwork/spec/backend/database-guidelines.md\` | ORM, migrations, query patterns, naming conventions |
| \`.superwork/spec/backend/error-handling.md\` | How errors are caught, logged, and returned |
| \`.superwork/spec/backend/logging-guidelines.md\` | Log levels, format, what to log |
| \`.superwork/spec/backend/quality-guidelines.md\` | Code review standards, testing requirements |
`;
...
`;
  ...
  if (packages && packages.length > 0) {
    ...
    for (const pkg of packages) {
      ...
      if (pkgType !== "frontend") {
        content += `\n- Backend guidelines: \`.superwork/spec/${specName}/backend/\`\n`;
      }
      if (pkgType !== "backend") {
        content += `\n- Frontend guidelines: \`.superwork/spec/${specName}/frontend/\`\n`;
      }
    }
...
function getBootstrapTaskJson(
  ...
  if (packages && packages.length > 0) {
    ...
    relatedFiles = packages.map(
      (pkg) => `.superwork/spec/${sanitizePkgName(pkg.name)}/`,
    );
  } else if (projectType === "frontend") {
    subtasks = [
    ...
    relatedFiles = [".superwork/spec/frontend/"];
  } else if (projectType === "backend") {
    subtasks = [
      { name: "Fill backend guidelines", status: "pending" },
      { name: "Add code examples", status: "pending" },
    ];
    relatedFiles = [".superwork/spec/backend/"];
  } else {
    // fullstack
    subtasks = [
      { name: "Fill backend guidelines", status: "pending" },
    ...
    relatedFiles = [".superwork/spec/backend/", ".superwork/spec/frontend/"];
  }
  ...
  return {
    ...
    parent: null,
    relatedFiles,
    notes: `First-time setup task created by superwork init (${projectType} project)`,
    meta: {},
  };
}
...
export async function init(options: InitOptions): Promise<void> {
  ...
  const banner = figlet.textSync("Superwork", { font: "Rebel" });
  ...
  if (developerName) {
    ...
    console.log(
      chalk.gray(
        "\nSuperwork supports team collaboration - each developer has their own\n" +
          ...
          for (const pkg of detected) {
            const { specSource } = await inquirer.prompt<{
            ...
            }>([
              {
                ...
                choices: [
                  { name: "From scratch (Superwork default)", value: "blank" },
  ...
  if (selectedTemplate) {
    ...
    if (result.success) {
      ...
      const retryCmd = registry
        ? `superwork init --registry ${registry.gigetSource} --template ${selectedTemplate}`
        : `superwork init --template ${selectedTemplate}`;
    ...
    if (result.success) {
      ...
      console.log(
        chalk.gray(
          `   You can retry later: superwork init --registry ${registry.gigetSource}`,
        ),
      );
    }
  }
...
/**
 * Print "What We Solve" section showing Superwork value proposition
