...
import { copySuperworkDir } from "../templates/extract.js";

// Import superwork templates (generic, not project-specific)
import {
  workflowMdTemplate,
  configYamlTemplate,
 ...
 *
 * This function creates the .superwork/ directory structure by:
...
export async function createWorkflowStructure(
  ...
  // Create base .superwork directory
  ensureDir(path.join(cwd, DIR_NAMES.WORKFLOW));

  // Copy scripts/ directory from templates
  await copySuperworkDir("scripts", path.join(cwd, PATHS.SCRIPTS), {
    executable: true,
  });
