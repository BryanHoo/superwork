...
# - .superwork/workspace/{developer}/features/{name}/ -> .superwork/tasks/{MM}-{DD}-{name}-{developer}/
...
# Check if .superwork exists
if [[ ! -d "$SUPERWORK_DIR" ]]; then
    echo "Error: .superwork directory not found in $PROJECT_DIR"
    exit 1
fi

# Determine the workspace directory (could be workspace or agent-traces if not yet updated)
if [[ -d "$SUPERWORK_DIR/workspace" ]]; then
    WORKSPACE_DIR="$SUPERWORK_DIR/workspace"
elif [[ -d "$SUPERWORK_DIR/agent-traces" ]]; then
    WORKSPACE_DIR="$SUPERWORK_DIR/agent-traces"
    echo "Warning: Found agent-traces instead of workspace. Run 'superwork update' first."
