...
- Codex 小改动：`$before-dev` -> `$tdd-core` -> `$check`
- Codex 非平凡任务：`$brainstorm` -> `$spec-plan` -> `$execute-plan`
- Codex Bug 排查：`$debug-root-cause` -> `$tdd-core` -> `$break-loop`
...
- Codex 会安装对应的 `$spec-plan`、`$execute-plan`、`$tdd-core`、`$debug-root-cause` 项目技能。
